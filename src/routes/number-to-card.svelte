...
<section in:fade={{ duration: 300, delay: 300 }} out:fade={{ duration:300 }} style="height: calc(100vh - 4rem)" class="flex flex-col justify-center items-center space-y-16">
	...
	<div class="h-full">
		<ChipPlatter type="card" max={max} hintStore={hint} />
		...
		<div class="flex justify-center items-center space-x-4 pt-8">
			...
			<input id="maxiumum" type="number" bind:value={max} min="1" max="52" class="bg-indigo-50 py-3 pl-[0.8rem] text-center rounded-lg border-indigo-400 focus:ring-indigo-400 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-100">
		</div>
	</div>
</section>